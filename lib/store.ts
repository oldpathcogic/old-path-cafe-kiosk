...
export type MenuItem = { id:string; name:string; category:string; description:string; imageUrl:string; priceCents:number; available:boolean; statusLabel:string; featured:boolean; displayOrder:number; allowedAddons:string[]; options:string[]; showOnKiosk:boolean; showOnMenuBoard:boolean };
...
const seedItems:MenuItem[] = [
  {id:"regular-coffee",name:"Regular Coffee",category:"Coffee",description:"Classic drip coffee, hot and fresh",imageUrl:"/menu-images/regular-coffee.webp",priceCents:400,available:true,statusLabel:"",featured:false,displayOrder:1,allowedAddons:["vanilla","caramel","hazelnut","mocha","oat-milk","almond-milk"],options:["Regular","Decaf"],showOnKiosk:true,showOnMenuBoard:true},
  {id:"cold-brew",name:"Cold Brew",category:"Coffee",description:"Smooth, bold, and refreshing",imageUrl:"/menu-images/cold-brew.webp",priceCents:600,available:true,statusLabel:"Limited Edition",featured:true,displayOrder:2,allowedAddons:["vanilla","caramel","hazelnut","mocha","oat-milk","almond-milk"],options:[],showOnKiosk:true,showOnMenuBoard:true},
  {id:"latte",name:"Latte",category:"Coffee",description:"Hot or iced, creamy and rich",imageUrl:"/menu-images/latte.webp",priceCents:600,available:true,statusLabel:"",featured:false,displayOrder:3,allowedAddons:["vanilla","caramel","hazelnut","mocha","oat-milk","almond-milk","extra-shot","whipped-cream","decaf-espresso"],options:["Hot","Iced"],showOnKiosk:true,showOnMenuBoard:true},
  {id:"cappuccino",name:"Cappuccino",category:"Coffee",description:"Foamy, smooth, and full of flavor",imageUrl:"/menu-images/cappuccino.webp",priceCents:600,available:true,statusLabel:"",featured:false,displayOrder:4,allowedAddons:["vanilla","caramel","hazelnut","mocha","oat-milk","almond-milk","extra-shot","decaf-espresso"],options:[],showOnKiosk:true,showOnMenuBoard:true},
  {id:"espresso-shot",name:"Espresso Shot",category:"Coffee",description:"Strong and rich",imageUrl:"/menu-images/espresso-shot.webp",priceCents:250,available:true,statusLabel:"",featured:false,displayOrder:5,allowedAddons:["decaf-espresso"],options:[],showOnKiosk:true,showOnMenuBoard:true},
  {id:"donut",name:"Donut",category:"Pastries",description:"Sweet pastry",imageUrl:"/menu-images/donut.webp",priceCents:200,available:true,statusLabel:"",featured:false,displayOrder:6,allowedAddons:[],options:[],showOnKiosk:true,showOnMenuBoard:true},
  {id:"croissant",name:"Croissant",category:"Pastries",description:"Buttery, flaky pastry",imageUrl:"/menu-images/croissant.webp",priceCents:200,available:true,statusLabel:"",featured:false,displayOrder:7,allowedAddons:[],options:[],showOnKiosk:true,showOnMenuBoard:true},
];
...
const settingDefaults:Record<string,string> = {cafeName:"She Brews",subtitle:"Coffee. Connection. Community.",footer:"All proceeds support the ministry",paymentEnabled:"false",paymentProvider:"none",itemSheetUrl:"",addonSheetUrl:"",ordersLedgerUrl:""};
...
export async function getMenu(){
  ...
  return {
    items:itemRows.results.map(r=>({id:String(r.id),name:String(r.name),category:String(r.category),description:String(r.description),imageUrl:String(r.image_url||`/menu-images/${String(r.id)}.webp`),priceCents:Number(r.price_cents),available:bool(r.available),statusLabel:String(r.status_label||""),featured:bool(r.featured),displayOrder:Number(r.display_order),allowedAddons:list(r.allowed_addons),options:list(r.options),showOnKiosk:bool(r.show_on_kiosk),showOnMenuBoard:bool(r.show_on_menu_board)})),
