...
export async function GET(){
  try{
    await getMenu();
    const rows=await db().prepare("SELECT pickup_code,updated_at FROM orders WHERE status='ready' AND pickup_code IS NOT NULL ORDER BY updated_at ASC LIMIT 12").all<Record<string,unknown>>();
